...
def issued_item(request, pk):
    ...
    if request.method == 'POST':
        ...
        if sales_form.is_valid():
            ...
            # if 'quantity' in request.POST and request.POST['quantity'].isnumeric():
            issued_quantity = int(request.POST['quantity'])
            issued_item.total_quantity -= int(issued_quantity)
            issued_item.save()
...
def reciept_detail(request, reciept_id):
    reciepts = Sale.objects.get(id = reciept_id)
    return render(request, 'products/reciept_detail.html', {'reciepts': reciepts})
